...
public struct ChatScreenView: View {
    ...
    /// Initializes the chat screen view with bindings and initial messages.
    ///
    /// - Parameters:
    ///   - isBlurred: A binding that determines if the UI should be blurred.
    ///   - initialMessages: The list of initial messages to populate the screen.
    public init(
        isBlurred: Binding<Bool>,
        initialMessages: [MessageResponseModel]
    ) {
        self._isBlurred = isBlurred
        self._viewModel = StateObject(wrappedValue: ChatScreenViewModel(initialMessages: initialMessages))
    }
}
...
extension ChatScreenView {
    public var body: some View {
        ZStack(alignment: .topTrailing) {
            VStack {
                ProfileImageView(imageName: config.images.profileImageName,
        ...
        VStack(spacing: 8) {
            Button(action: {
                ...
                Text(viewModel.getDeleteMessageCount())
                    ...
                    .background(config.colors.deleteAlertBackgroundColor)
                    .foregroundColor(.red)
                    .font(.headline)
                    .cornerRadius(10)
            })
